...
def add_SP_liked_albums(liked_albums: list) -> list:
    ...
    '''
    if liked_albums:
        print(f"\Adding Spotify saved albumds to YouTube Music library...")
        not_added_albums = []
        for album in liked_albums:
            ...
            for res in yt_search_res:
                ...
                if res["artists"][0]["name"] == album_artist:
                    found[res_browse_ID] += SCORE_COEFFICIENT
            ...
            if found:
                ...
                res_album = YTM_CLIENT.get_album(res_browse_ID)
                res_playlist_ID = res_album["audioPlaylistId"]
                YTM_CLIENT.rate_playlist(res_playlist_ID, "LIKE")
...
def print_not_added(keep_dupes: bool) -> None:
    ...
    if NOT_ADDED:
        ...
        for playlist in NOT_ADDED:
            ...
            for song_query in NOT_ADDED[playlist]["unfound"]:
        ...
        if not keep_dupes:
            ...
            for playlist in NOT_ADDED:
                print(f"-----PLAYLIST: {playlist}-----")
                for song_query in NOT_ADDED[playlist]["dupes"]:
                    print(f"{index}. {song_query}")
                    index += 1
            if index == 1:
